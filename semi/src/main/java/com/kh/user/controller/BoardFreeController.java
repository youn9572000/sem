...
public class BoardFreeController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		listCount = new BoardService().selectFreeCount(1);
		...
		List<Board> list = new BoardService().selectFree(pi, sort);
		request.setAttribute("list", list);
		request.setAttribute("pi", pi);
		
		request.getRequestDispatcher("/views/userPage/board/boardFree.jsp").forward(request, response);
	}
