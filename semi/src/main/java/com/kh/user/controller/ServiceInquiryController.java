...
/**
 * Servlet implementation class serviceInquiryController
 */
@WebServlet("/service/inquiry")
public class ServiceInquiryController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		listCount = new InquiryService().selectInquiryCount(4);
		...
		List<Board> list = new InquiryService().selectInquiry(pi, sort);
		request.setAttribute("list", list);
		request.setAttribute("pi", pi);
		
		request.getRequestDispatcher("/views/userPage/board/serviceInquiry.jsp").forward(request, response);
	}
