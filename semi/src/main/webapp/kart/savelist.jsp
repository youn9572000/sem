...
<!--header-->
    ...
    %>
	<div class="productListWrap" style="none;">
		<ul>
			<% 
            for (Save item : saveList) { 
        %>
			<li class="product_list" style="width:236px;"><img src="<%= item.getProductImage() %>" alt="상품 이미지"
				style="none";> 	     <!-- 상품 이름 -->
       		 <p class="product-name"><%= item.getProductName() %></p>

				  <!-- 상품 가격 -->
        	 <p class="product-price"><c:out value=`\${image.PRODUCT_PRICE}`/>원</p>
