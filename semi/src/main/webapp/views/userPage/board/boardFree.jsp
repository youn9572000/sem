...
<body>
	...
	<main>
		<section class="notice-section">
			...
			<div class="search-bar">
				<div class="sort-dropdown">
					...
					<div class="sort-options">
						<a href="<%= request.getContextPath() %>/board/free?cpage=1&sort=date">날짜순</a>
    					<a href="<%= request.getContextPath() %>/board/free?cpage=1&sort=count">조회수순</a>
    					<a href="<%= request.getContextPath() %>/board/free?cpage=1&sort=plus">추천순</a>
					</div>
				</div>
			...
			<div class="pagination">
				<% if (currentPage > 1) { %>
			        <!-- 이전 페이지 이동 -->
			        <button class="page-btn" onclick="location.href='<%= request.getContextPath() %>/board/free?cpage=1'">&lt;&lt;</button>
			        <button class="page-btn" onclick="location.href='<%= request.getContextPath() %>/board/free?cpage=<%= (currentPage > 1) ? (currentPage - 1) : 1 %>'"<%= (currentPage <= 1) ? "disabled" : "" %>>&lt;</button>
			    <% } %>
			    ...
			    <% if (currentPage < maxPage) { %>
			        <!-- 다음 페이지 이동 -->
			        <button class="page-btn" onclick="location.href='<%= request.getContextPath() %>/board/free?cpage=<%= (currentPage < maxPage) ? (currentPage + 1) : maxPage %>'"<%= (currentPage >= maxPage) ? "disabled" : "" %>>&gt;</button>
			        <button class="page-btn" onclick="location.href='<%= request.getContextPath() %>/board/free?cpage=<%= maxPage %>'">&gt;&gt;</button>
			    <% } %>
			</div>
		</section>
	</main>
	...
	<script>
		function movePage(cpage){
			location.assign('<%= request.getContextPath() %>/board/free/list?cpage='+cpage);
		}
